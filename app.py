...
def generate_patient_report():
    try:
        ...
        # 1. Encabezado
        elements.append(Paragraph("INFORME CLÍNICO DE ENDOMETRIOSIS", header_style))
        elements.append(Paragraph(f"Generado el: {datetime.now().strftime('%d/%m/%Y %H:%M')}", normal_style))
        elements.append(Spacer(1, 24))
        
        # 2. Información del paciente
        elements.append(Paragraph("INFORMACIÓN DEL PACIENTE", section_style))
        
        patient_data = [
            [Paragraph("<b>Nombre:</b>", bold_style), Paragraph(data['personal']['full_name'], normal_style)],
            [Paragraph("<b>RUT:</b>", bold_style), Paragraph(data['personal']['id_number'], normal_style)],
            [Paragraph("<b>Fecha de Nacimiento:</b>", bold_style), Paragraph(data['personal']['birth_date'], normal_style)],
            [Paragraph("<b>Edad:</b>", bold_style), Paragraph(f"{data['personal']['age']} años", normal_style)],
            [Paragraph("<b>Tipo de Sangre:</b>", bold_style), Paragraph(data['personal']['blood_type'], normal_style)],
            [Paragraph("<b>Previsión:</b>", bold_style), Paragraph(data['personal']['insurance'], normal_style)],
            [Paragraph("<b>ID Clínico:</b>", bold_style), Paragraph(data['personal'].get('clinic_id', 'N/A'), normal_style)]
        ]
        ...
        # 3. Historia Clínica
        elements.append(Paragraph("HISTORIA CLÍNICA", section_style))
        elements.append(Spacer(1, 6))
        
        # 3.1 Antecedentes Médicos
        elements.append(Paragraph("<b>Antecedentes Médicos:</b>", bold_style))
        
        medical_history = [
            [Paragraph("<b>Cirugías Ginecológicas:</b>", bold_style), Paragraph("Sí" if data['history']['gynecological_surgery'] else "No", normal_style)],
            [Paragraph("<b>Enf. Inflamatoria Pélvica:</b>", bold_style), Paragraph("Sí" if data['history']['pelvic_inflammatory'] else "No", normal_style)],
            [Paragraph("<b>Quistes Ováricos:</b>", bold_style), Paragraph("Sí" if data['history']['ovarian_cysts'] else "No", normal_style)],
            [Paragraph("<b>Endometriosis Familiar:</b>", bold_style), Paragraph("Sí" if data['history']['family_endometriosis'] else "No", normal_style)],
            [Paragraph("<b>Enf. Autoinmunes Familiar:</b>", bold_style), Paragraph("Sí" if data['history']['family_autoimmune'] else "No", normal_style)],
            [Paragraph("<b>Cáncer Familiar:</b>", bold_style), Paragraph("Sí" if data['history']['family_cancer'] else "No", normal_style)],
            [Paragraph("<b>Medicamentos:</b>", bold_style), Paragraph(data['history']['medications'] or "No registrado", normal_style)]
        ]
        
        medical_table = Table(medical_history, colWidths=[150, 270])
        ...
        # 3.2 Historia Menstrual
        elements.append(Paragraph("<b>Historia Menstrual:</b>", bold_style))
        
        menstrual_history = [
            [Paragraph("<b>Edad de Menarquia:</b>", bold_style), Paragraph(f"{data['menstrual']['menarche_age']} años", normal_style)],
            [Paragraph("<b>Duración del Ciclo:</b>", bold_style), Paragraph(f"{data['menstrual']['cycle_length']} días", normal_style)],
            [Paragraph("<b>Duración del Período:</b>", bold_style), Paragraph(f"{data['menstrual']['period_duration']} días", normal_style)],
            [Paragraph("<b>Última Menstruación:</b>", bold_style), Paragraph(data['menstrual']['last_period'], normal_style)],
            [Paragraph("<b>Nivel de Dolor:</b>", bold_style), Paragraph(f"{data['menstrual']['pain_level']}/10", normal_style)],
            [Paragraph("<b>Dolor Crónico:</b>", bold_style), Paragraph("Sí" if data['menstrual']['pain_chronic'] else "No", normal_style)]
        ]
        
        menstrual_table = Table(menstrual_history, colWidths=[150, 270])
        ...
        elements.append(Paragraph("RESULTADOS DE EXÁMENES", section_style))
        elements.append(Spacer(1, 6))
        
        # 4.1 Biomarcadores
        elements.append(Paragraph("<b>Biomarcadores:</b>", bold_style))
        
        biomarkers_data = [
            [Paragraph("<b>CA-125:</b>", bold_style), Paragraph(f"{data['biomarkers']['ca125']} U/mL" if data['biomarkers']['ca125'] is not None else "No medido", normal_style)],
            [Paragraph("<b>IL-6:</b>", bold_style), Paragraph(f"{data['biomarkers']['il6']} pg/mL" if data['biomarkers']['il6'] is not None else "No medido", normal_style)],
            [Paragraph("<b>TNF-α:</b>", bold_style), Paragraph(f"{data['biomarkers']['tnf_alpha']} pg/mL" if data['biomarkers']['tnf_alpha'] is not None else "No medido", normal_style)],
            [Paragraph("<b>VEGF:</b>", bold_style), Paragraph(f"{data['biomarkers']['vegf']} pg/mL" if data['biomarkers']['vegf'] is not None else "No medido", normal_style)],
            [Paragraph("<b>AMH:</b>", bold_style), Paragraph(f"{data['biomarkers']['amh']} ng/mL" if data['biomarkers']['amh'] is not None else "No medido", normal_style)],
            [Paragraph("<b>PCR:</b>", bold_style), Paragraph(f"{data['biomarkers']['crp']} mg/L" if data['biomarkers']['crp'] is not None else "No medido", normal_style)]
        ]
        
        biomarkers_table = Table(biomarkers_data, colWidths=[100, 320])
        ...
        # 4.2 Imágenes
        elements.append(Paragraph("<b>Resultados de Imágenes:</b>", bold_style))
        imaging_text = data['biomarkers']['imaging'] or "No se realizaron estudios de imágenes"
        elements.append(Paragraph(imaging_text, normal_style))
        
        if data['biomarkers']['imaging_details']:
            elements.append(Spacer(1, 4))
            elements.append(Paragraph("<b>Detalles:</b>", bold_style))
            elements.append(Paragraph(data['biomarkers']['imaging_details'], normal_style))
        ...
        exam_data = [
            [Paragraph("<b>Estatura:</b>", bold_style), Paragraph(f"{data['examination']['height']} cm" if data['examination']['height'] else "No registrado", normal_style)],
            [Paragraph("<b>Peso:</b>", bold_style), Paragraph(f"{data['examination']['weight']} kg" if data['examination']['weight'] else "No registrado", normal_style)],
            [Paragraph("<b>IMC:</b>", bold_style), Paragraph(f"{data['examination']['bmi']}" if data['examination']['bmi'] else "No calculado", normal_style)],
            [Paragraph("<b>Examen Pélvico:</b>", bold_style), Paragraph(data['examination']['pelvic_exam'] or "No registrado", normal_style)],
            [Paragraph("<b>Examen Vaginal:</b>", bold_style), Paragraph(data['examination']['vaginal_exam'] or "No registrado", normal_style)],
            [Paragraph("<b>Notas Clínicas:</b>", bold_style), Paragraph(data['examination']['clinical_notes'] or "No registrado", normal_style)]
        ]
        
        exam_table = Table(exam_data, colWidths=[120, 300])
        ...
        elements.append(Paragraph("EVALUACIÓN DE RIESGO DE ENDOMETRIOSIS", section_style))
        elements.append(Spacer(1, 6))
        ...
        evaluation_data = [
            ...
            [Paragraph("<b>Nivel de Riesgo:</b>", bold_style), 
             Paragraph(risk_level, ParagraphStyle('RiskLevel', parent=normal_style, textColor=risk_color))],
            
            [Paragraph("<b>Factores Clave:</b>", bold_style), 
             Paragraph(', '.join(explanation['key_factors']) or 'No identificados', normal_style)]
        ]

        evaluation_table = Table(evaluation_data, colWidths=[150, 270])
        ...
        signature_data = [
            ["", ""],
            ["__________________________", "__________________________"],
            ["Firma del Paciente", "Firma del Profesional Tratante"]
        ]
        ...
        # Preparar respuesta
        buffer.seek(0)
        filename = f"Informe_Endometriosis_{data['personal']['full_name'].replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.pdf"
        response = make_response(buffer.getvalue())
        response.headers['Content-Type'] = 'application/pdf'
        response.headers['Content-Disposition'] = f'attachment; filename={filename}'
        ...
        app.logger.error(f"Error generando informe: {str(e)}")
